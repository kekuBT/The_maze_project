...
void drawSprite(void)
{
	...
	int splx = (int)sp[3].x >> 6, spy = (int)sp[3].y >> 6;
	/* normal grid position plus offset */
	int splx_add = ((int)sp[3].x + 15) >> 6, spy_add = ((int)sp[3].y + 15) >> 6;
	/* // normal grid position subtract offset */
	int splx_sub = ((int)sp[3].x - 15) >> 6, spy_sub = ((int)sp[3].y - 15) >> 6;

	if (sp[3].x > plx && mapW[spy * 8 + splx_sub] == 0)
	{
		sp[3].x -= 0.06 * fps;
	}
	if (sp[3].x < plx && mapW[spy * 8 + splx_add] == 0)
	{
		sp[3].x += 0.06 * fps;
	}
	if (sp[3].y > py && mapW[spy_sub * 8 + splx] == 0)
	{
		sp[3].y -= 0.06 * fps;
	}
	if (sp[3].y < py && mapW[spy_add * 8 + splx] == 0)
	{
		sp[3].y += 0.06 * fps;
	}
...
void display(void)
{
	...
	if (gameState == 2)
	{
		...
		int iplx = plx / 64.0, iplx_add_xo = (plx + xo) / 64.0, iplx_sub_xo = (plx - xo) / 64.0;
		/* y position and offset */
		int ipy = py / 64.0, ipy_add_yo = (py + yo) / 64.0, ipy_sub_yo = (py - yo) / 64.0;
		/* move forward */
		if (Keys.w == 1)
		{
			if (mapW[ipy * mapX + iplx_add_xo] == 0)
			{
				plx += pdx * 0.2 * fps;
			}
			if (mapW[ipy_add_yo * mapX + iplx] == 0)
			{
				py += pdy * 0.2 * fps;
			}
		}
		...
		if (Keys.s == 1)
		{
			if (mapW[ipy * mapX + iplx_sub_xo] == 0)
			{
				plx -= pdx * 0.2 * fps;
			}
			if (mapW[ipy_sub_yo * mapX + iplx] == 0)
			{
				py -= pdy * 0.2 * fps;
			}
		}
...
void ButtonDown(unsigned char key, int x, int y)
{
	...
	if (key == 'o' && sp[0].state == 0)
	{
		...
		int iplx = plx / 64.0, iplx_add_xo = (plx + xo) / 64.0;
		int ipy = py / 64.0, ipy_add_yo = (py + yo) / 64.0;

		if (mapW[ipy_add_yo * mapX + iplx_add_xo] == 4)
		{
			mapW[ipy_add_yo * mapX + iplx_add_xo] = 0;
		}
	}
